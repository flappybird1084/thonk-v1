...
from train_script_v3 import TrainConfig, build_model
...
CKPT_PATH = os.path.join(CFG.ckpt_dir, CFG.sft_ckpt_name)
if not os.path.exists(CKPT_PATH):
    CKPT_PATH = os.path.join(CFG.ckpt_dir, CFG.pretrain_ckpt_name)
...
CONTROL_TOKEN_IDS = {tid for tok in ("[STARTOFTEXT]", "[INST]", "[/INST]") for tid in encode(tok)}
...
@torch.no_grad()
def generate_until_eot(prompt: str, max_new_tokens: int, temperature: float, top_k: int) -> str:
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="Thonk v3 Generator") as demo:
        gr.Markdown("## Thonk v3 Text Generator")
        prompt = gr.Textbox(label="Prompt", lines=6, placeholder="Ask a question or give an instruction...")
        with gr.Row():
