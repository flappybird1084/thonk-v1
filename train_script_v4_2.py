...
class TrainConfig:
    ...
    dpo_ckpt_name: str = "v4_dpo_ultrafeedback-2.pth"
...
def sequence_logprob(model: nn.Module, input_ids: torch.Tensor, labels: torch.Tensor):
...
def run_dpo_stage(
    ...
    ref_model = build_model(cfg).to(cfg.device)
    ref_model.load_state_dict(model.state_dict())
    ref_model.eval()
    for param in ref_model.parameters():
        param.requires_grad_(False)

    dataset = UltraFeedbackDPODataset(cfg.block_size)
    loader_kwargs = {
    ...
    use_amp = cfg.device == "cuda"
    pbar = tqdm(total=cfg.dpo_steps, ncols=120, desc="dpo_ultrafeedback")
    data_iter = iter(dataloader)
    optimizer.zero_grad(set_to_none=True)

    for step in range(1, cfg.dpo_steps + 1):
        running_loss = 0.0
        running_acc = 0.0
        for _ in range(cfg.grad_accum_steps):
            try:
            ...
            chosen_x = chosen_x.to(cfg.device, non_blocking=True)
            chosen_y = chosen_y.to(cfg.device, non_blocking=True)
            rejected_x = rejected_x.to(cfg.device, non_blocking=True)
            rejected_y = rejected_y.to(cfg.device, non_blocking=True)
            ...
            with amp_ctx:
                policy_chosen_logp = sequence_logprob(train_model, chosen_x, chosen_y)
                policy_rejected_logp = sequence_logprob(
                    train_model, rejected_x, rejected_y
                )
            ...
            logits = cfg.dpo_beta * (policy_logratio - ref_logratio)
        ...
        if step % cfg.log_every == 0:
            pbar.set_postfix(
                loss=f"{(running_loss / cfg.grad_accum_steps):.4f}",
                pref_acc=f"{(running_acc / cfg.grad_accum_steps):.3f}",
                lr=f"{scheduler.get_last_lr()[0]:.2e}",
            )
...
def main():
    ...
    if cfg.resume_sft2_if_available and os.path.exists(sft2_ckpt):
        ...
        model.load_state_dict(torch.load(sft2_ckpt, map_location=cfg.device))
    else:
    ...
    if cfg.resume_dpo_if_available and os.path.exists(dpo_ckpt):
        ...
        model.load_state_dict(torch.load(dpo_ckpt, map_location=cfg.device))
    else:
        print("Stage 4: DPO on UltraFeedback (prompt/chosen/rejected)")
        run_dpo_stage(
            model=model,
            train_model=train_model,
            cfg=cfg,
